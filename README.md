<div id="top"></div>

<br />
<div align="center">

<!-- 🔰 BADGES -->
![GitHub repo size](https://img.shields.io/github/repo-size/codewith-salman/article-preview-component)
![GitHub stars](https://img.shields.io/github/stars/codewith-salman/article-preview-component?style=social)
![GitHub forks](https://img.shields.io/github/forks/codewith-salman/article-preview-component?style=social)

[![Live Demo](https://img.shields.io/badge/Live%20Demo-View%20Project-brightgreen?style=for-the-badge&logo=vercel)](https://article-preview-component-xi-wine.vercel.app/)

[![GitHub Follow](https://img.shields.io/github/followers/codewith-salman?style=social)](https://github.com/codewith-salman)
[![LinkedIn](https://img.shields.io/badge/LinkedIn-Muhammad%20Salman%20Arshad-blue?style=social&logo=linkedin)](https://www.linkedin.com/in/muhammad-salmanarshad/)
[![Figma](https://img.shields.io/badge/Figma-codewithsalman-black?style=social&logo=figma)](https://www.figma.com/@codewithsalman)
[![Email](https://img.shields.io/badge/Email-msalmanwebdev%40gmail.com-red?style=social&logo=gmail)](mailto:msalmanwebdev@gmail.com)

<br />

<!-- 🔰 PROJECT LOGO -->
<img src="Article%20Preview/images/favicon-32x32.png" alt="Article Preview Component Logo" width="120"/>

<br />
<br />

<h1 align="center">📰 Article Preview Component</h1>

A clean and interactive **Article Preview UI Component** built using  
**HTML, CSS, and JavaScript**.  
Designed to display article summaries with a modern layout and an engaging share-button interaction.

<a href="https://article-preview-component-xi-wine.vercel.app/"><strong>➥ Live Demo</strong></a>

</div>

---

<!-- TABLE OF CONTENTS -->
<details>
  <summary>📑 Table of Contents</summary>
  <ol>
    <li><a href="#about-the-project">About The Project</a></li>
    <li><a href="#features">Features</a></li>
    <li><a href="#built-with">Built With</a></li>
    <li><a href="#contact">Contact</a></li>
  </ol>
</details>

---

## 📖 About The Project

<img src="Article%20Preview/images/desktop.png" alt="Article Preview Component Desktop View" width="100%" />

The **Article Preview Component** is a minimal and modern UI element that displays an article summary along with author details and interactive share options.  
This component is perfect for **blogs, news portals, magazine websites, or UI practice projects** where users want to show article teasers in an elegant and compact way.

The design focuses on clarity, readability, and smooth interactions. By clicking the share icon, users can reveal a sleek popup containing social media sharing options — a valuable UI pattern commonly used in modern content-based websites.

This component is ideal for developers practicing **responsive design, layout structuring, and interactive UI behavior** using pure HTML, CSS, and JavaScript.

<p align="right">(<a href="#top">back to top</a>)</p>

---

## ✨ Features

- Clean and modern UI  
- Responsive layout for all screen sizes  
- Interactive share popup with smooth animation  
- Author info section with image and details  
- Minimal, content-focused design  
- Fully customizable for any article or blog layout  

<p align="right">(<a href="#top">back to top</a>)</p>

---

## 🛠️ Built With

- **HTML5**  
- **CSS3**  
- **JavaScript (Vanilla)**  

<p align="right">(<a href="#top">back to top</a>)</p>

---

## 📬 Contact

**Muhammad Salman Arshad**

- 💼 **LinkedIn:** [Muhammad Salman Arshad](https://www.linkedin.com/in/muhammad-salmanarshad/)  
- 🎨 **Figma:** [codewithsalman](https://www.figma.com/@codewithsalman)  
- 📧 **Email:** [msalmanwebdev@gmail.com](mailto:msalmanwebdev@gmail.com)

<p align="right">(<a href="#top">back to top</a>)</p>
